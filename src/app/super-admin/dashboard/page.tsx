...
function EditFutsalForm({ futsal, onUpdate, onCancel, setNotification }: { futsal: Futsal, onUpdate: (data: FormData) => void, onCancel: () => void, setNotification: React.Dispatch<React.SetStateAction<{message: string, type: 'success' | 'info'} | null>> }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label>Add Images: <input type="file" accept="image/*" multiple onChange={(e) => setImages(e.target.files)} /></label>
      </div>
      <div>
  ...
  return (
    <div className="border rounded-lg p-6 bg-white">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <textarea placeholder="Description" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className="w-full p-2 border rounded" />
        <div>
          <label>Images (up to 5): <input type="file" accept="image/*" multiple onChange={(e) => setImages(e.target.files)} /></label>
        </div>
        <div>
          <label>Video: <input type="file" accept="video/*" onChange={(e) => setVideo(e.target.files?.[0] || null)} /></label>
        </div>
